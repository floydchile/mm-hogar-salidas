...
st.set_page_config(page_title="MyM Hogar - Central Omnicanal", layout="wide")
...
try:
    ...
    if not df.empty:
        ...
        if st.button("🚀 Sincronizar Canales Activos"):
            ...
            # 2. MeLi (Filtro por SKU de prueba)
            if "XXXG42" in str(sku_sel):
                if sync_meli_stock(stk_val): st.success("✅ Mercado Libre: OK")
                else: st.error("❌ Mercado Libre: Error")
